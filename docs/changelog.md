...
## [2025-01-27] - Исправление застревания карточек после отпускания

### Исправлено

- Исправлена проблема с застреванием карточек на месте после отпускания
- Улучшена логика возврата карточки в поток marquee
- Добавлен сброс прогресса анимации для корректного перезапуска
- Увеличена длительность анимации возврата для более плавного перехода

### Технические детали

- Возврат карточки теперь происходит одновременно по X и Y с `x: 0, y: 0`
- Добавлен `loopRef.current.progress(0)` для сброса прогресса анимации marquee
- Увеличена длительность анимации возврата до 0.4s
- Добавлена задержка 100ms перед перезапуском marquee для корректной синхронизации

## [2025-01-27] - Исправление синхронизации карточек с marquee после драга

### Исправлено

- Исправлена проблема с "зависанием" карточек после отпускания
- Карточки теперь корректно возвращаются в поток marquee
- Улучшена логика возврата с двухэтапной анимацией
- Восстановлен правильный селектор для избежания дублирования

### Технические детали

- Изменена логика `onRelease`: сначала возврат по Y, затем сброс X через `gsap.set(el, { x: 0 })`
- Уменьшена длительность анимации возврата по Y до 0.3s для более быстрого отклика
- Селектор восстановлен на `.experts__item[data-draggable='true']` для точного поиска
- Карточки теперь корректно синхронизируются с бесконечной прокруткой marquee

## [2025-01-27] - Исправление проблем с возвратом карточек и дублированием

### Исправлено

- Исправлена проблема с возвратом карточек влево после драга
- Устранено дублирование Draggable при использовании класса `.expert-card`
- Улучшен селектор для поиска только оригинальных карточек
- Исправлена логика сохранения позиции X при возврате карточки

### Технические детали

- Селектор изменен на `.experts__item[data-draggable='true'] .expert-card` для точного поиска только драгуемых карточек
- В `onRelease` добавлено сохранение текущей позиции X через `gsap.getProperty(el, "x")`
- Теперь карточки возвращаются только по Y, сохраняя позицию в потоке marquee
- Устранено дублирование Draggable экземпляров для клонированных карточек

## [2025-01-27] - Улучшение работы Draggable в компоненте экспертов

### Исправлено

- Улучшена работа драга карточек экспертов для более адекватного поведения
- Исправлен селектор для поиска только оригинальных карточек с атрибутом `data-draggable="true"`
- Добавлены параметры `inertia: true` и `throwResistance: 2000` для более естественного движения
- Уменьшены ограничения по Y с 120px до 80px для более комфортного драга
- Улучшена анимация возврата с `back.out(1.2)` и увеличена длительность до 0.4s

### Технические детали

- Селектор изменен на `.experts__item[data-draggable='true']` для точного поиска только драгуемых карточек
- Добавлена инерция для более естественного поведения при отпускании карточки
- Уменьшены ограничения по вертикали для более комфортного взаимодействия
- Улучшена анимация возврата карточки на исходную позицию

## [2025-01-27] - Исправление дублирования Draggable в компоненте экспертов

### Исправлено

- Устранено дублирование Draggable экземпляров для карточек экспертов
- Исправлена логика селектора для поиска только оригинальных карточек
- Добавлена проверка на существование Draggable перед созданием нового экземпляра
- Убрана некорректная проверка `destroyers.length > 0` которая всегда была false

### Технические детали

- Изменен селектор с `.expert-card` на `.experts__list:first-child .expert-card` для поиска только оригинальных карточек
- Добавлена проверка `(el as any)._gsDraggable` для предотвращения создания дублирующих экземпляров
- Исправлена логика в useEffect для корректной работы с массивом destroyers
- Теперь Draggable создается только для оригинальных карточек, а не для клонов

## [2025-01-27] - Исправление анимации блюра в этапах конкурса

### Исправлено

- Исправлена анимация блюра для чисел в компоненте "Этапы конкурса"
- Анимация `blur-numbers` теперь применяется к правильному элементу `.stage-card__number`
- Исправлен селектор для запуска анимации при добавлении класса `.active`
- Убрана неправильная анимация с `.stage-card__number-wrap`

### Технические детали

- Анимация блюра теперь корректно запускается для всех чисел при появлении секции в viewport
- Исправлен CSS селектор: `.stage-card__numbers.active .stage-card__number` вместо `.stage-card__number-wrap`
- Анимация применяется к самим числам, а не к их контейнеру
- Сохранены параметры анимации: `animation-duration: 0.5s`, `animation-delay: var(--transition-slower-3)`
