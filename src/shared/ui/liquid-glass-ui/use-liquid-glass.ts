...
export const useLiquidGlass = () => {
  ...
  const buildDisplacementImage = useCallback(() => {
    ...
    if (debugPen.current) {
      ...
      const svgElement = debugPen.current.querySelector(".displacement-image");
      if (svgElement && svgEl.current) {
        const serialized = new XMLSerializer().serializeToString(svgElement);
        const encoded = encodeURIComponent(serialized);
        const dataUri = `data:image/svg+xml,${encoded}`;

        const feImage = svgEl.current.querySelector("feImage");
        if (feImage) {
